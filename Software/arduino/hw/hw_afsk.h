 ...
 */
#define AFSK_STROBE_INIT() do { DDRB |= BV(1); } while (0) //Changed from B5 (D13) to B3 (D11) 
#define AFSK_RX_LED_INIT() do { DDRB |= BV(2); } while (0) //Changed from B5 (D13) to B3 (D11) 

/*
 * Set the pin high. This macro is called at the beginning of the interrupt routine
 */
#define AFSK_STROBE_ON()   do { PORTB |= BV(1); } while (0)
#define AFSK_RX_LED_ON()   do { PORTB |= BV(2); } while (0)

/*
 * Set the pin low. This macro is called at the end of the interrupt routine
 */
#define AFSK_STROBE_OFF()  do { PORTB &= ~BV(1); } while (0)
#define AFSK_RX_LED_OFF()  do { PORTB &= ~BV(2); } while (0)
 ...
 */
#define AFSK_DAC_INIT(ch, ctx)   do { (void)ch, (void)ctx; DDRD |= 0xF0; DDRB |= BV(3); DDRB |= BV(1);} while (0)  //LED on PB1 (D9) and PTT is on PB3 (D11)

/**
 * Start DAC convertions on channel \a ch.
 * \param ch DAC channel.
 */
#define AFSK_DAC_IRQ_START(ch)   do { (void)ch; extern bool hw_afsk_dac_isr; PORTB |= BV(3); hw_afsk_dac_isr = true; PORTB |= BV(1);} while (0)

/**
 * Stop DAC convertions on channel \a ch.
 * \param ch DAC channel.
 */
#define AFSK_DAC_IRQ_STOP(ch)    do { (void)ch; extern bool hw_afsk_dac_isr; PORTB &= ~BV(3); hw_afsk_dac_isr = false; PORTB &= ~BV(1);} while (0)
