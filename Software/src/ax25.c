...
{
	unsigned len = MIN(sizeof(addr->call), strnlen(addr->call, sizeof(addr->call)));
	...
	uint8_t ssid = (addr->h_bit ? 0x80 : 0) | 0x60 | (addr->ssid << 1) | (last ? 0x01 : 0);
